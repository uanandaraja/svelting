...
export const streamdownTheme = {
	...
	// Image styling - hide download button
	img: {
		container: "my-4",
		base: "max-w-full rounded-lg",
		downloadButton: "hidden",
	},
	// Mermaid styling - hide download button
	mermaid: {
		base: "my-4",
		downloadButton: "hidden",
	},
	...
	table: {
		base: "w-full text-sm",
		container: "my-4 overflow-x-auto rounded-lg border border-border",
	},
	thead: { base: "bg-muted/50" },
	tbody: { base: "" },
	tfoot: { base: "bg-muted/30 font-medium" },
	tr: { base: "border-b border-border last:border-0" },
	th: { base: "px-4 py-2 text-left font-semibold text-foreground" },
	td: { base: "px-4 py-2 text-muted-foreground" },
} as const;
